...
debug, error, warning, info, success, riemannStream = [None] * 6
    ...
    def __init__(self, host, port):
        ...
        try:
            ...
            self.client.send({
                ...
                'state': 'ok',
                'service': "alibuild started"
            })
            self.enabled = True
            info("Sending log data to %s:%s" % (host, port))
        except Exception as e:
            msg = (
                "RIEMANN_HOST {host}:{port} specified, "
                "however there was a problem initialising:"
...
class LogFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if record.levelno == logging.BANNER and sys.stdout.isatty():
            ...
            return "\n\033[1;34m==>\033[m \033[1m{msg}\033[m".format(msg=msg)
        elif record.levelno in (logging.INFO, logging.BANNER):
            return record.msg

        levelname = self.LEVEL_COLORS.get(record.levelno, self.COLOR_RESET)
        levelname += record.levelname
        levelname += self.COLOR_RESET

        formatted = []
        for msg in record.msg.split("\n"):
            formatted.append(
                self.fmtstr.format(levelname=levelname, message=msg)
            )
...
class ProgressPrint:
    ...
    def end(self, msg="", error=False):
        ...
        self.erase()
        if msg:
            msg = ": %s%s\033[m" % ("\033[31m" if error else "\033[32m", msg)
...
# If the RiemannStreamer can be used, we add it to those use during printout.
