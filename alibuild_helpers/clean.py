...
def decideClean(workDir, architecture, aggressiveCleanup):
    ...
    """
    latest_links = glob.glob("%s/BUILD/*-latest*" % workDir)
    symlinksBuild = [os.readlink(x) for x in latest_links]
    ...
    for x in installedPackages:
        latest = glob.glob(x + "/latest*")
        symlinksInstall += [path.realpath(y) for y in latest]

    toDelete += [
        x for x in glob.glob(installGlob + "*")
...
def doClean(workDir, architecture, aggressiveCleanup, dryRun):
    ...
    to_delete = "\n".join(toDelete)
    finalMsg = "This will delete the following directories:\n\n" + to_delete

    if dryRun:
        finalMsg += "\n\n--dry-run / -n specified. Doing nothing."
        print_results(finalMsg)
        sys.exit(0)
