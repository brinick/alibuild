...
def generate_analytics_id():
    ...
    cmd = "uuidgen >  ~/.config/alibuild/analytics-uuid"
    err, output = getstatusoutput(cmd)
    # If an error is found while generating the unique user ID,
    # we disable the analytics on the machine.
    if err:
...
def askForAnalytics():
    msg = ("In order to improve user experience, aliBuild would "
           "like to gather analytics about your builds.\nYou can "
...
def report(eventType, **metadata):
    ...
    desc = "aliBuild/{vers} ({os_type}; {os_proc} {os_vers}) Python/{py_vers}"
    desc = desc.format(
        vers=os.environ["ALIBUILD_VERSION"],
        os_type=ostype,
        os_proc=osprocessor,
        os_vers=osversion,
        py_vers=".".join([str(x) for x in sys.version_info[:3]])
    )
    args = ["curl", "--max-time", "5", "--user-agent", desc]
