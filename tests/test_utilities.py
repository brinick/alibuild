...
class TestUtilities(unittest.TestCase):
    def test_osx(self):
        for payload in architecturePayloads:
            result, hasOsRelease, osReleaseLines, \
                platformTuple, platformSystem, platformProcessor = payload
            self.assertEqual(
                result,
                doDetectArch(hasOsRelease,
                             osReleaseLines,
                             platformTuple,
                             platformSystem,
                             platformProcessor)
    ...
    def test_UTF8_Hasher(self):
        ...
        self.assertEqual(
            h1.hexdigest(),
            "2af8e41129115eb231a0af76ec5465d3a9184fc4"
        )
        self.assertEqual(
            h2.hexdigest(),
    ...
    def test_filterByArchitecture(self):
        Case = namedtuple("Case", "arch have expect")
        cases = [
            Case("osx_x86-64", ["AliRoot"], ["AliRoot"]),
            Case("osx_x86-64", ["AliRoot:(?!osx)"], []),
            Case("osx_x86-64", ["AliRoot:(?!osx)", "GCC"], ["GCC"]),
            Case("osx_x86-64", ["AliRoot:(?!slc6)", "GCC"], ["AliRoot", "GCC"]),
            Case("osx_x86-64", ["AliRoot:slc6", "GCC:osx"], ["GCC"]),
            Case("osx_x86-64", [], [])
        ]
        ...
        for case in cases:
            self.assertEqual(
                case.expect,
                list(filterByArchitecture(case.arch, case.have))
            )

    @patch("alibuild_helpers.utilities.getstatusoutput")
    def test_dockerStatusOutput(self, mock_getstatusoutput):
        cmd = dockerStatusOutput(cmd="echo foo",
                                 dockerImage="image",
                                 executor=mock_getstatusoutput)
        self.assertEqual(
            mock_getstatusoutput.mock_calls,
