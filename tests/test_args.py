...
ARCHITECTURE_ERROR = [
    call((
        u"Unknown / unsupported architecture: foo.\n\n"
        "On Linux, x86-64:\n   "
        ...
        "Alternatively, there is the `--force-unknown-architecture' option."))
]
...
class ArgsTestCase(unittest.TestCase):
    @patch('alibuild_helpers.args.commands')
    def test_actionParsing(self, mock_commands):
        def side_effect(x):
            return GETSTATUSOUTPUT_MOCKS[x]
        mock_commands.getstatusoutput.side_effect = side_effect
    ...
    @patch('alibuild_helpers.args.argparse.ArgumentParser.error')
