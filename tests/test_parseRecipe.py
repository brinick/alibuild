...
from alibuild_helpers.utilities import (
    parseRecipe, getRecipeReader, parseDefaults,
    FileReader, GitReader, validateDefaults
)
...
class TestRecipes(unittest.TestCase):
    def test_recipes(self):
        ...
        err, meta, body = parseRecipe(BufferReader("test_broken_1.sh",
                                                   TEST_BROKEN_1))
        self.assertEqual(err,
                         "Unable to parse test_broken_1.sh. Header missing.")
        err, meta, body = parseRecipe(BufferReader("test_broken_2.sh",
                                                   TEST_BROKEN_2))
        self.assertEqual(err,
                         "Malformed header for test_broken_2.sh\nEmpty recipe.")
        self.assertTrue(not meta and not body)
        err, meta, body = parseRecipe(BufferReader("test_broken_3.sh",
                                                   TEST_BROKEN_3))
        self.assertEqual(err.encode("ascii"), ERROR_MSG_3.encode("ascii"))
        self.assertEqual(meta, None)
        self.assertEqual(body.strip(), "")
        err, meta, body = parseRecipe(BufferReader("test_broken_4.sh",
                                                   TEST_BROKEN_4))
        self.assertEqual(err, ERROR_MSG_4)
        err, meta, body = parseRecipe(BufferReader("test_broken_5.sh",
                                                   TEST_BROKEN_5))
        self.assertEqual(err, ERROR_MSG_5)
        err, meta, body = parseRecipe(BufferReader("test_broken_6.sh",
                                                   TEST_BROKEN_6))
    ...
    def test_parseDefaults(self):
        ...
        disable = ["bar"]
        err, overrides, taps = parseDefaults(disable, defsGetter, Recoder())

        self.assertEqual(disable, ["bar", "foo"])
        self.assertEqual(
            overrides,
            {'defaults-release': {}, 'root': {'requires': 'GCC'}}
        )
        self.assertEqual(taps, {'root': 'dist:ROOT@master'})

    def test_validateDefault(self):
        ok, out, validDefaults = validateDefaults({"something": True},
                                                  "release")
        self.assertEqual(ok, True)
        ok, out, validDefaults = validateDefaults(
            {"package": "foo", "valid_defaults": ["o2", "o2-dataflow"]},
            "release"
        )
        self.assertEqual(ok, False)
        self.assertEqual(
            out,
            ('Cannot compile foo with `release\' default. '
             'Valid defaults are:\n - o2\n - o2-dataflow')
        )
        ok, out, validDefaults = validateDefaults(
            {"package": "foo", "valid_defaults": ["o2", "o2-dataflow"]},
        ...
        self.assertEqual(ok, True)
        ok, out, validDefaults = validateDefaults(
            {"package": "foo", "valid_defaults": "o2-dataflow"},
            "o2"
        )
        self.assertEqual(ok, False)
        ...
        self.assertEqual(ok, True)
        ok, out, validDefaults = validateDefaults(
            {"package": "foo", "valid_defaults": 1},
            "o2"
        )
        self.assertEqual(ok, False)
        self.assertEqual(
            out,
            ('valid_defaults needs to be a string or '
             'a list of strings. Found [1].')
        )
        ok, out, validDefaults = validateDefaults(
            {"package": "foo", "valid_defaults": {}},
            "o2"
        )
        self.assertEqual(ok, False)
        self.assertEqual(
