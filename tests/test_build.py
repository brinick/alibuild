...
from alibuild_helpers.build import (
    doBuild, HttpRemoteSync, RsyncRemoteSync, NoRemoteSync
)
...
TEST_ROOT_BUILD_HASH = "1fc9be611d016280a2a66519923965839d0d009b"
if sys.version_info[0] < 3:
    TEST_ROOT_BUILD_HASH = "2cb80ea18ac9ec56b94c4d9a865caa1f7e03a8f0"
...
def dummy_getstatusoutput(x):
    if re.match("/bin/bash --version", x):
        msg = ("GNU bash, version 3.2.57(1)-release (x86_64-apple-darwin17)\n"
...
def dummy_readlink(x):
    k = "/sw/TARS/osx_x86-64/defaults-release/defaults-release-v1-1.osx_x86-64.tar.gz"
    v = "../../osx_x86-64/store/27/27ce49698e818e8efb56b6eff6dd785e503df341/defaults-release-v1-1.osx_x86-64.tar.gz"
    return {k: v}[x]
...
class BuildTestCase(unittest.TestCase):
    ...
    def test_coverDoBuild(
        self, mock_is_writeable, mock_getStatusOutputBash, mock_banner,
        ...
        def side_effect(x, **kwds):
            return dummy_execute(x, mock_git_clone, mock_git_fetch, **kwds)
        mock_execute.side_effect = side_effect
        mock_workarea_execute.side_effect = side_effect
        ...
        fail_msg = ("Expected only no calls to git fetch "
        ...
        fail_msg = ("Expected only one call to git clone "
                    "(called %d times instead)" % mock_git_clone.call_count)
        self.assertEqual(mock_git_clone.call_count, 1, fail_msg)
        fail_msg = ("Expected only one call to git fetch "
                    "(called %d times instead)" % mock_git_fetch.call_count)
    ...
    def test_coverSyncs(
        self, mock_error, mock_die, mock_sys,
        mock_getstatusoutput, mock_execute, mock_urlopen
    ):
        syncers = [
            NoRemoteSync(),
            HttpRemoteSync(remoteStore="https://local/test",
                           architecture="osx_x86-64",
                           workdir="/sw",
                           insecure=False),
            RsyncRemoteSync(remoteStore="ssh://local/test",
                            writeStore="ssh://local/test",
                            architecture="osx_x86-64",
                            workdir="/sw",
                            rsyncOptions="")
        ]
